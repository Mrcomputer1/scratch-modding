...
3. The category of the block, this is a integer. The list of valid categories can be found in the `Specs.as` in an array called `categories`. This is a motion block, which is category ID `1`. `100` is added to the category ID for the Stage version of a category.
...
3. `2` is the category of block. This block is a looks block, which is category ID `2`.
...
Our block doesn't do anything, since we haven't given it any code yet, so let's do that now. Implementations for the blocks are located in the `primitives` folder. The loosk primitives are located in the `LooksPrims.as` file.

First, we need to import `DialogBox` so we can actually display a alert box. Under the other `import`s near the top of the file add:
