...
Like other blocks, we add our block's function to `getPrimitives`, but we also add it to `getHats` this time. If the category you want to add the block to has no hats already, it won't have the `getHats` method, however it is easy to add based off the one in `scratch3_events.js`:
...
:::info
Scratch VM doesn't normally interact with the browser, instead Scratch GUI handles interacting with browser and communicates with Scratch GUI via events and Scratch VM's IO device system. However, for the purposes of this tutorial, it is easier to do it directly in Scratch VM.
:::
...
| `scratch-vm`      | `src/blocks/scratch3_events.js`           | [scratch3_events.js](resources/scratch3_events.js.txt)        |
